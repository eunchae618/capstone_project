...
const Map = () => {
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      try {
        // 지도 옵션 설정
        const mapOptions = {
          center: new window.naver.maps.LatLng(HALLYM_UNIV.lat, HALLYM_UNIV.lng),
          zoom: 15
        };
        ...
        if (container.offsetWidth === 0 || container.offsetHeight === 0) {
          console.warn('지도 컨테이너 크기가 0입니다. 크기를 강제로 설정합니다.');
    ...
    const checkNaverMaps = () => {
      ...
      const checkInterval = setInterval(() => {
        ...
        if (window.naver && window.naver.maps && window.naver.maps.Map) {
          ...
          // 인증 실패 원인 분석
          if (!window.naver) {
            console.error('원인: window.naver가 정의되지 않음 - 스크립트 로드 실패 또는 인증 실패');
          } else if (!window.naver.maps) {
            console.error('원인: window.naver.maps가 정의되지 않음 - Maps API 초기화 실패');
          } else if (!window.naver.maps.Map) {
            console.error('원인: window.naver.maps.Map이 정의되지 않음 - Map 클래스 로드 실패');
          }
    ...
    script.onload = () => {
      console.log('네이버 지도 신규 Maps API 스크립트 로드 완료, 초기화 대기 중...');
      checkNaverMaps();
    };
  ...
  const searchPlaces = async (keyword, mapInstance) => {
    ...
    // 검색어에 위치 정보가 없으면 추가
    if (!searchQuery.includes('춘천') && !searchQuery.includes('강원') && !searchQuery.includes('한림대')) {
    ...
    if (!window.naver || !window.naver.maps) {
      console.error('네이버 지도 API가 로드되지 않았습니다.');
      alert('네이버 지도 API가 아직 로드되지 않았습니다. 잠시 후 다시 시도해주세요.');
      return;
    }
      ...
      window.naver.maps.Service.geocode({
        ...
        try {
          ...
          if (!response) {
            ...
            alert('검색 결과를 받을 수 없습니다.\n\n주소 형식으로 검색해주세요.\n예: "춘천시 한림대", "강원도 춘천시 한림대학교"');
            return;
          }

          // ProgressEvent나 Event인 경우 (네트워크 오류)
          if (response instanceof ProgressEvent || response instanceof Event) {
          ...
          setIsSearching(false);
          alert('검색 결과를 처리하는 중 오류가 발생했습니다.\n\n주소 형식으로 다시 검색해주세요.');
        }
      });
    } catch (error) {
  ...
  const handleSearchApiResults = async (items, mapInstance) => {
    ...
    items.forEach((item, index) => {
      ...
      window.naver.maps.Service.geocode({
        ...
        if (status === window.naver.maps.Service.Status.OK && geoResponse && geoResponse.result && geoResponse.result.items && geoResponse.result.items.length > 0) {
          ...
          const infoWindow = new window.naver.maps.InfoWindow({
            content: `
              <div style="padding: 12px; min-width: 200px; font-family: 'Gmarket Sans TTF', sans-serif;">
                <div style="font-weight: bold; margin-bottom: 6px; font-size: 15px; color: #002546;">${item.title.replace(/<[^>]*>/g, '')}</div>
                <div style="font-size: 12px; color: #666; margin-bottom: 4px;">${item.address.replace(/<[^>]*>/g, '')}</div>
                ${item.category ? `<div style="font-size: 11px; color: #999;">${item.category}</div>` : ''}
                ${item.telephone ? `<div style="font-size: 11px; color: #999; margin-top: 4px;">📞 ${item.telephone}</div>` : ''}
              </div>
            `
          });
      ...
      const infoWindow = new window.naver.maps.InfoWindow({
        content: `
          <div style="padding: 12px; min-width: 180px; font-family: 'Gmarket Sans TTF', sans-serif;">
            <div style="font-weight: bold; margin-bottom: 6px; font-size: 14px; color: #002546;">${item.address}</div>
            <div style="font-size: 12px; color: #666;">${item.roadAddress || item.address}</div>
          </div>
        `
      });
  ...
  const handlePlaceClick = (place) => {
    if (map && window.naver) {
      const position = new window.naver.maps.LatLng(place.position.lat, place.position.lng);
      ...
      const marker = markers.find(m => {
        ...
        return Math.abs(markerPos.lat() - place.position.lat) < 0.0001 &&
      ...
      <div className="map-container">
        <div className="map-content">
          <div className="map-area">
            <div ref={mapRef} className="naver-map" style={{ width: '100%', height: '100%' }}></div>
            ...
            <div className="search-top-overlay">
              <div className="search-box-top">
                ...
                />
                {isSearching && <div className="search-loading-top">검색 중...</div>}
                <button 
                ...
                >
                  {isSearching ? (
                    ...
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#040404" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="category-section">
              ...
              {selectedCategory === '별점' && (
                <div className="sort-section">
                  ...
                  <div className="sort-options">
                    <button
                      className={`sort-option ${sortOrder === '높은 순' ? 'active' : ''}`}
                    ...
                    <button
                      className={`sort-option ${sortOrder === '낮은 순' ? 'active' : ''}`}
              ...
              <div className="category-list">
                <button
                  className={`category-item ${selectedCategory === '별점' ? 'active' : ''}`}
