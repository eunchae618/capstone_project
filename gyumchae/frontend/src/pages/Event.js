...
const Event = () => {
  ...
  const handleSpin = () => {
    ...
    const targetSectorCenter = selectedPrizeIndex * sectorAngle + sectorAngle / 2;
    ...
    if (rouletteElement) {
      rouletteElement.style.setProperty('--final-rotation', `${finalRotation}deg`);
          ...
          <div className="camera-section">
            <div className="upload-area">
              <div className="camera-box">
                {selectedImage ? (
                  <img src={selectedImage} alt="영수증" className="receipt-image" />
                ) : (
                  <div className="camera-placeholder">
                    <svg className="upload-icon" width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                      <polyline points="17 8 12 3 7 8"></polyline>
                      <line x1="12" y1="3" x2="12" y2="15"></line>
                    </svg>
                    <p className="camera-text">영수증 이미지를 업로드하세요</p>
                    <p className="camera-subtext">클릭하거나 드래그하여 파일을 선택</p>
                  </div>
                )}
              </div>
              <input
              ...
              <div className="roulette-overlay">
                {!isVerified ? (
                  <>
                    ...
                    <p className="overlay-message">영수증 인증 후<br />룰렛을 돌릴 수 있습니다</p>
                  </>
                ) : (
                  <>
                    <div className="overlay-icon">✓</div>
                    <p className="overlay-message">이미 룰렛을<br />돌리셨습니다</p>
                  </>
                )}
              </div>
            )}
            <div className="roulette-wrapper-inline">
              ...
              <div className="roulette-pointer">
                <svg width="40" height="50" viewBox="0 0 60 80" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M30 80 L0 20 L30 30 L60 20 Z" fill="#FF4444" stroke="#ffffff" strokeWidth="2"/>
                  <path d="M30 30 L30 0" stroke="#FF4444" strokeWidth="4" strokeLinecap="round"/>
                </svg>
              </div>
              <div className={`roulette-container ${!canSpin ? 'disabled' : ''}`}>
                <svg 
                  className={`roulette-svg ${isSpinning ? 'spinning' : ''}`}
                  viewBox="0 0 400 400"
                >
                  <circle cx="200" cy="200" r="200" fill="#ffffff" stroke="rgba(255, 255, 255, 0.9)" strokeWidth="6"/>
                  {prizes.map((prize, index) => {
                    ...
                    const textX = 200 + 130 * Math.cos((textAngle - 90) * Math.PI / 180);
                    const textY = 200 + 130 * Math.sin((textAngle - 90) * Math.PI / 180);
                    ...
                    return (
                      <g key={prize.id}>
                        ...
                        <text
                          ...
                          transform={`rotate(${textAngle} ${textX} ${textY - 8})`}
                        >
                          {prize.name}
                        </text>
                  ...
                  <circle cx="200" cy="200" r="60" fill="#d9d9d9" stroke="rgba(255, 255, 255, 0.9)" strokeWidth="3"/>
