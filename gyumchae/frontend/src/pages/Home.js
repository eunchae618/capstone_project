...
const Home = () => {
  ...
  const stores = [
    ...
    { id: 4, name: '동신냉삼집', image: 'r4_img.webp' },
  ...
  const handlePrevStore = () => {
    const newIndex = selectedStoreIndex === 0 
      ? stores.length - 1 
  ...
  const handleNextStore = () => {
    const newIndex = selectedStoreIndex === stores.length - 1 
      ? 0 
      : selectedStoreIndex + 1;
    setSelectedStoreIndex(newIndex);
  };
        ...
        <div className="hero-content">
          <div className="search-section">
            <div className="search-box">
              <p className="search-placeholder">관심있는 가게를 빠르게 확인해보세요.</p>
              <div 
      ...
      <div className="content-section">
        ...
        <div className="stores-slider-container">
          ...
          >
            {repeatedStores.map((store, index) => {
              ...
              return (
                <div
                  key={`${store.id}-${index}`}
                  className={`store-card ${isCenter ? 'store-card-selected' : ''} store-card-pos-${index - currentDisplayIndex}`}
          ...
          <div className="store-navigation">
            ...
            >
              <img src={`${process.env.PUBLIC_URL}/images/page.png`} alt="이전 가게" />
            </button>
            <div className="store-indicator">{selectedStoreIndex + 1} / {stores.length}</div>
            <button 
              className="nav-arrow nav-arrow-right"
              onClick={handleNextStore}
            >
              <img src={`${process.env.PUBLIC_URL}/images/page.png`} alt="다음 가게" />
            </button>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="apply-content">
          <div className="apply-text-section">
            ...
            <p className="apply-subtitle">이 거리의 주인공은 바로 사장님입니다.</p>
            <p className="apply-description">가게 등록으로 함께 성장하는 상권의 일원이 되어주세요.</p>
          </div>
          <Link to="/apply" className="apply-button">APPLICATION FORM</Link>
        </div>
      </div>
